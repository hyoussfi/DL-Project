...
# Define path to IMG_CLASSES folder
original_dataset_path = os.path.join(path, "IMG_CLASSES")
print(f"Original dataset path: {original_dataset_path}")

# Copy dataset to current directory (beside the script)
current_dir = os.path.dirname(os.path.abspath(__file__))
local_dataset_path = os.path.join(current_dir, "skin_dataset")

# Create the directory if it doesn't exist
if not os.path.exists(local_dataset_path):
    print(f"Creating local dataset directory at: {local_dataset_path}")
    os.makedirs(local_dataset_path, exist_ok=True)
    
    # Copy the dataset (only if it hasn't been copied before)
...
# Use the local dataset path
dataset_path = local_dataset_path

# Print the folders (classes) in the dataset
...
train_transform = transforms.Compose([
    transforms.Resize((224, 224)),  # EfficientNetB0 uses smaller resolution
    transforms.RandomHorizontalFlip(),
    transforms.RandomRotation(15),
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
    transforms.RandomResizedCrop(224, scale=(0.85, 1.0)),  # Smaller size for B0
    transforms.ToTensor(),
...
full_dataset = ImageFolder(root=dataset_path, transform=None)  # No transform initially
...
# Create data loaders - increased batch size for B0
batch_size = 32  # Can use larger batch size with B0
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=4)

# Define the EfficientNetB0 model with custom classifier
def create_efficientnet_model(num_classes, fine_tune=True):
    # Load pre-trained EfficientNetB0
    model = models.efficientnet_b0(weights='IMAGENET1K_V1')
    
    # Freeze early layers for fine-tuning
    if fine_tune:
        # Freeze first 70% of layers (B0 has fewer layers)
    ...
    # Replace the classifier - EfficientNetB0 has 1280 final features
    in_features = model.classifier[1].in_features  # Should be 1280 for B0
    model.classifier = nn.Sequential(
        nn.Dropout(p=0.3, inplace=True),  # Lower dropout for smaller model
...
optimizer = optim.AdamW([
    {'params': [p for n, p in model.named_parameters() if 'classifier' not in n], 'lr': 0.00005},  # Higher LR for B0
    {'params': model.classifier.parameters(), 'lr': 0.0005}  # Higher LR for new classifier
], weight_decay=1e-4)  # Reduced weight decay for B0
